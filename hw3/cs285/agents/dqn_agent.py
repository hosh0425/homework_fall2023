...
class DQNAgent(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values = reward[:,None] + self.discount*next_q_values*((1-done.int())[:,None])

        # TODO(student): train the critic with the target values
        qa_values = self.critic(obs)
        q_values = torch.gather(qa_values, 1, action[:,None]) # Compute from the data actions; see torch.gather
